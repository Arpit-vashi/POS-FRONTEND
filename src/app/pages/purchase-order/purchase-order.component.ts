...
export class PurchaseOrderComponent implements OnInit {
  ...
  loadProductsBySupplier(supplierId: string): void {
    ...
    if (!isNaN(numericSupplierId)) {
      this.productService.getProductsBySupplierId(numericSupplierId).subscribe(
        (products: any[]) => {
          const productOptions = products.map(product => ({ label: product.name, value: product.name }));
          this.products = productOptions;
          this.purchaseOrderForm.patchValue({ products: productOptions });
        },
        (error) => {
  ...
  deletePurchaseOrder(orderId: number) {
    if (confirm('Are you sure you want to delete this purchase order?')) {
      this.purchaseOrderService.deletePurchaseOrder(orderId).subscribe(
        () => {
          this.purchaseOrders = this.purchaseOrders.filter(po => po.id !== orderId);
          this.showToast('success', 'Success', 'Purchase order deleted successfully.');
          this.getAllPurchaseOrders();
        },
        (error) => {
          console.error('Error deleting purchase order:', error);
          this.showToast('error', 'Error', 'Failed to delete purchase order.');
          this.getAllPurchaseOrders();
        }
      );
    }
    ...
    if (this.purchaseOrderForm.valid) {
      ...
      this.purchaseOrderService.createPurchaseOrder(formData).subscribe(
        ...
        (error) => {
          ...
          this.showToast('error', 'Error', 'Failed to create purchase order.');
        }
      );
    } else {
  ...
  onRowEditSave(purchaseOrder: PurchaseOrderResponse) {
    ...
    this.purchaseOrderService.updatePurchaseOrder(orderId, purchaseOrder).subscribe(
      () => {
        this.showToast('success', 'Success', 'Purchase order updated successfully.');
      },
      (error) => {
        console.error('Error updating purchase order:', error);
        this.showToast('error', 'Error', 'Failed to update purchase order.');
      }
    );
  }

  onRowEditCancel(purchaseOrder: PurchaseOrderResponse, index: number) {
    this.purchaseOrders[index] = this.clonedPurchaseOrders[purchaseOrder.id];
