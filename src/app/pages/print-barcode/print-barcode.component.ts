...
export class PrintBarcodeComponent implements OnInit {
    ...
    async printSelectedProducts(): Promise<void> {
        ...
        for (const product of this.selectedProducts3) {
            ...
            div.innerHTML = `
        <div>
          <img id="barcode-${product.productId}" src="${this.getBarcodeImage(
                product.barcodeImage
            )}" width="700" height="50" style="margin-top: 10px;">
        </div>
      `;
            ...
            try {
                ...
                const lines = doc.splitTextToSize(
                    `${product.productId}, ${product.name}, Barcode Number: ${product.barcodeNumber}, Price: ${product.price}`,
                    maxLineWidth
                );
        ...
        this.messageService.add({
            severity: 'success',
            summary: 'Success',
            detail: 'Barcode List Printed successfully',
        });
    }
