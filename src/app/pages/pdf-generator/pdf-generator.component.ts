...
export class PdfGeneratorComponent {
  ...
  generatePDF(): void {
    ...
    while (rowsLeft > 0) {
      ...
      // Move page number to bottom right corner
      ...
      const numberOfRowsThatFit = Math.floor((pageHeight - startY - 40) / 20);

      const startIndex = (currentPage - 1) * numberOfRowsThatFit;
      const endIndex = Math.min(startIndex + numberOfRowsThatFit, this.tableData.length);
      ...
      doc.setFontSize(10);
      doc.text(`${dateTimeText}${this.getCurrentDateTime()}${user}`, pageWidth / 2, pageHeight - 10, { align: 'center' });
